...
function EventDetail({
  ...
  useEffect(() => {
    ...
    getCommentsByEventId({ page: 0, pageSize: 5, id: params.id }).then(
      (res) => {
  ...
  return (
    <>
      ...
      <Card
        ...
        style={cardStyle}
        cover={
          <img src="https://i.dunya.com/2/1280/720/storage/files/images/2021/01/13/arcelik-jbvS_cover.jpg.webp" />
        }
        actions={[
          roleInfo === admin ? (
            <div>
      ...
      >
        <Meta title={event.title} description={event.body} />
        <Divider />
        ...
        <div style={{display:"inline-block",marginLeft: "87%",marginTop: "10px"}}>
          <Pagination
            defaultCurrent={1}
            total={10}
            onChange={(page, pageSize) => getCommentsByEventId(page, pageSize)}
          />
        </div>
      </Card>
    </>
